...
def add_address(vendor_id: str, request: schema.VendorAddress, db: Session = Depends(get_db)):
...
def get_address(vendor_id: str, db: Session = Depends(get_db)):
    address = db.query(model.VendorAddress).filter(model.VendorAddress.vendor_id == vendor_id).all()
    return address


@router.put('/update_vendor_address/{vendor_id}/{address_id}')
def update_address(vendor_id: str, address_id: str, request: schema.VendorAddress, db: Session = Depends(get_db)):
    address_query = db.query(model.VendorAddress).filter(model.VendorAddress.address_id == address_id,
                                                         model.VendorAddress.vendor_id == vendor_id)
...
def add_address(customer_id: str, request: schema.CustomerAddress, db: Session = Depends(get_db)):
...
def get_address(customer_id: str, db: Session = Depends(get_db)):
    address = db.query(model.CustomerAddress).filter(model.CustomerAddress.customer_id == customer_id).all()
    return address


@router.put('/update_customer_address/{customer_id}/{address_id}')
def update_address(customer_id: str, address_id: str, request: schema.CustomerAddress, db: Session = Depends(get_db)):
    address_query = db.query(model.CustomerAddress).filter(model.CustomerAddress.address_id == address_id,
